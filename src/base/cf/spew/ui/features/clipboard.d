...
interface Feature_Management_Clipboard {
    @property {
        ...
        ///
        managed!string clipboardText(IAllocator alloc = theAllocator(), Duration timeout = 0
                .seconds) shared;
        ///
...
@property {
    ...
    size_t maxClipboardDataSize(scope shared(Management_UserInterface) self) {
        if (!self.capableOfClipboard)
            ...
            return (cast(shared(Have_Management_Clipboard))self).__getFeatureClipboard()
                .maxClipboardDataSize();
        }
    }
    ...
    void maxClipboardDataSize(scope shared(Management_UserInterface) self, size_t amount) {
        if (!self.capableOfClipboard)
            ...
            (cast(shared(Have_Management_Clipboard))self).__getFeatureClipboard()
                .maxClipboardDataSize(amount);
        }
    }
...
managed!string clipboardText(scope shared(Management_UserInterface) self,
        IAllocator alloc = theAllocator(), Duration timeout = 0.seconds) {
    if (!self.capableOfClipboard)
        return managed!string.init;
    else {
        return (cast(shared(Have_Management_Clipboard))self).__getFeatureClipboard()
            .clipboardText(alloc, timeout);
    }
}
