...
@property {
	 ...
	 */
    void cursor(T)(managed!T self, WindowCursorStyle to)
    ...
    WindowCursorStyle cursor(T)(managed!T self)
            if (is(T : IWindow) || is(T : IWindowCreator)) {
        if (!self.capableOfCursors)
	 ...
	 */
    void cursorIcon(T)(managed!T self, scope ImageStorage!RGBA8 to, vec2!ushort hotspot)
            if (is(T : IWindow) || is(T : IWindowCreator)) {
        if (self.capableOfCursors) {
	 ...
	 */
    managed!(ImageStorage!RGBA8) cursorIcon(T)(managed!T self, IAllocator alloc = theAllocator())
            if (is(T : IWindow) || is(T : IWindowCreator)) {
        if (!self.capableOfCursors)
	 ...
	 * 		self	=	Window or window creator instance
	 */
    bool lockCursorToWindow(T)(managed!T self)
            if (is(T : IWindow) || is(T : IWindowCreator)) {
        if (self.capableOfCursors) {
    ...
    bool lockCursorToWindow(T)(managed!T self, WindowCursorStyle to)
            if (!(is(T : IWindow) || is(T : IWindowCreator))) {
        static assert(0,
                "I do not know how to handle " ~ T.stringof ~
                " I can only use IWindow or IWindowCreator.");
	 ...
	 * 		self	=	Window or window creator instance
	 */
    void unlockCursorFromWindow(T)(managed!T self)
            if (is(T : IWindow) || is(T : IWindowCreator)) {
        if (self.capableOfCursors) {
	 ...
	 */
    bool capableOfCursors(T)(managed!T self)
            if (is(T : IWindow) || is(T : IWindowCreator)) {
        if (self is null)
            return false;
        else {
