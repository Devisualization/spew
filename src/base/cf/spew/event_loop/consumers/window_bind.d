...
class WindowBind : EventLoopConsumer {
    ...
    @property {
        ///
        IWindow owner() shared {
            return cast(IWindow)owner_;
        }
        ///
        IWindow child() shared {
            return cast(IWindow)child_;
        }
        ...
        string description() shared {
            return "Binds two windows location + size together with an offset.";
        }

        Nullable!EventSource pairOnlyWithSource() shared {
            return Nullable!EventSource.init;
        }

        EventType pairOnlyWithEvents() shared {
            return Windowing_Events_Types.Prefix;
        }
    ...
    bool processEvent(ref Event event) shared {
        if (enabled && cast(size_t)event.wellData1Ptr == owner.__handle) {
            ...
            case Windowing_Events_Types.Window_Moved:
                child.location = vec2!int(
                        cast(int)(event.windowing.windowMoved.newX + offsetX),
            ...
            case Windowing_Events_Types.Window_Resized:
                if (bindSize) {
                    ...
                    // otherwise set size
                    if (event.windowing.windowResized.newWidth <= 0 ||
                            event.windowing.windowResized.newHeight <= 0) {
                        child.hide();
                        // yuckies
                    } else {
