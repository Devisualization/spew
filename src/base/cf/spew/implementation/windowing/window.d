...
abstract class WindowImpl : IWindow, IWindowEvents {
	...
	this(PlatformImpl platform) {
		...
		platform.windowToIdMapper[cast(size_t)__handle] = this;
	}

	~this() {
		platform.windowToIdMapper.remove(cast(size_t)__handle);
	}
...
version(Windows) {
	class WindowImpl_WinAPI : WindowImpl,
	...
	Have_ScreenShot, Have_Icon, Have_Menu, Have_Cursor, Have_Style {
		...
		@disable this(PlatformImpl platform);

		this(HWND hwnd, IContext context, IAllocator alloc, PlatformImpl platform, HMENU hMenu=null, bool processOwns=false) {
			super(platform);
		...
		@property {
			...
			managed!IDisplay display() {
				...
				if (monitor is null)
					...
					return cast(managed!IDisplay)managed!DisplayImpl_WinAPI(managers(), tuple(monitor, alloc, platform), alloc);
			}
