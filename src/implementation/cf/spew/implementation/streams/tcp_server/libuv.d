module cf.spew.implementation.streams.tcp_server.libuv;
import cf.spew.implementation.streams.tcp_server.base;
import cf.spew.implementation.streams.tcp_client.libuv;
...
final class LibUVTCPServer : AnTCPServer {
    ...
    static managed!ISocket_TCPServer create(Address address,
            ushort listBacklogAmount = 64, IAllocator alloc = theAllocator()) {
        ...
        if (libuv.uv_listen(&ret.ctx_stream, ret.listBacklogAmount_,
                &onStreamServerConnectCB) != 0) {
            alloc.dispose(ret);
            return managed!ISocket_TCPServer.init;
        }
...
extern (C) {
    void onStreamServerConnectCB(uv_stream_t* server, int status) {
        ...
        LibUVTCPSocket endpoint = serverSelf.alloc.make!LibUVTCPSocket(serverSelf,
                serverSelf.alloc);
