module cf.spew.implementation.streams.tcp_client.libuv;
import cf.spew.implementation.streams.tcp_client.base;
import cf.spew.streams.tcp;
import devisualization.bindings.libuv;
import devisualization.util.core.memory.managed;
import std.socket : Address, AddressFamily, sockaddr, InternetAddress,
    Internet6Address;
import core.time : Duration;
import stdx.allocator : IAllocator, theAllocator, make, dispose, makeArray;

final class LibUVTCPSocket : AnTCPSocket {
    @property {
        ...
        void keepAliveTimeout(Duration duration) {
            ...
            if (seconds > 0)
                libuv.uv_tcp_keepalive(&ctx_tcp, 1, cast(uint)seconds);
            else
                libuv.uv_tcp_keepalive(&ctx_tcp, 0, 0);
        }
    ...
    static managed!ISocket_TCP create(Address address, IAllocator alloc = theAllocator()) {
        ...
        if (libuv.uv_tcp_connect(&ret.uv_connect, &ret.ctx_tcp,
                cast(sockaddr*)&ret.addrstorage, &streamTCPCreateCB) != 0) {
            alloc.dispose(ret);
            return managed!ISocket_TCP.init;
        }
    ...
    void write(const(ubyte[]) data...) {
        ...
        libuv.uv_write(&writebuf.req, &ctx_stream, cast(const)&writebuf.buf,
                1, &streamTCPWriteCB);
    }
