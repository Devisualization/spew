module cf.spew.implementation.streams.udp.libuv;
import cf.spew.implementation.streams.udp.base;
...
final class LibUVUDPLocalPoint : AnUDPLocalPoint {
    ...
    bool joinMulticastGroup(scope string multicastAddress, scope string interfaceAddress = null) {
        return handleMulticastGroup(multicastAddress, interfaceAddress,
                uv_membership.UV_JOIN_GROUP);
    }

    bool leaveMulticastGroup(scope string multicastAddress, scope string interfaceAddress = null) {
        return handleMulticastGroup(multicastAddress, interfaceAddress,
                uv_membership.UV_LEAVE_GROUP);
    }

    bool handleMulticastGroup(scope string multicastAddress,
            scope string interfaceAddress, uv_membership membership) {
        if (multicastAddress.length == 0)
            return false;

        char[] theCopyMulticast, theCopyInterface;
        string toSendMulticast = multicastAddress, toSendInterface = interfaceAddress;
        ...
        int ret = libuv.uv_udp_set_membership(&ctx_udp, cast(const(char)*)toSendMulticast.ptr,
                cast(const(char)*)toSendInterface.ptr, membership);
    ...
    @property {
        ...
        bool multicastInterface(scope string input) {
            ...
            int ret = libuv.uv_udp_set_multicast_interface(&ctx_udp,
                    cast(const(char)*)toSend.ptr);
    ...
    static managed!ISocket_UDPLocalPoint create(Address address, IAllocator alloc = theAllocator()) {
        if (address is null || alloc is null)
        ...
        if (address.addressFamily == AddressFamily.INET) {
            ...
            assert(0, "Unknown address format");
    ...
    managed!Address localAddress(IAllocator alloc = theAllocator()) {
        ...
        if (!isOpen || libuv.uv_udp_getsockname(&ctx_udp, cast(sockaddr*)&addr, &alen) != 0) {
            ...
            return cast(managed!Address)managed!InternetAddress(
                    alloc.make!InternetAddress(*cast(sockaddr_in*)&addr), managers(), alloc);
        } else if (addr.ss_family == AF_INET6) {
            return cast(managed!Address)managed!Internet6Address(
                    alloc.make!Internet6Address(*cast(sockaddr_in6*)&addr), managers(), alloc);
        }
    ...
    managed!ISocket_UDPEndPoint connectTo(Address address, IAllocator alloc = theAllocator()) {
        ...
        if (address is null || alloc is null)
            return managed!ISocket_UDPEndPoint.init;
        ...
        return cast(managed!ISocket_UDPEndPoint)managed!LibUVUDPEndPoint(managers(),
                tuple(addr_storage, this, alloc), alloc);
    }
...
final class LibUVUDPEndPoint : AnUDPEndPoint {
    package(cf.spew.implementation) {
    ...
    void write(const(ubyte[]) data...) {
        ...
        version (all) {
            ...
            libuv.uv_udp_send(&writebuf.udpsend, &localPoint_.ctx_udp,
                    &writebuf.buf, 1, cast(sockaddr*)&addrstorage, &streamUDPWriteCB);
        } else {
            writebuf.req.data = writebuf;
            libuv.uv_write(&writebuf.req, &localPoint_.ctx_stream,
                    cast(const)&writebuf.buf, 1, &streamUDPWriteCB);
        }
    }
    ...
    managed!Address remoteAddress(IAllocator alloc = theAllocator()) {
        if (!isOpen) {
            ...
            return cast(managed!Address)managed!InternetAddress(alloc.make!InternetAddress(
                    *cast(sockaddr_in*)&addrstorage), managers(), alloc);
        } else if (addrstorage.ss_family == AF_INET6) {
            return cast(managed!Address)managed!Internet6Address(alloc.make!Internet6Address(
                    *cast(sockaddr_in6*)&addrstorage), managers(), alloc);
        }
...
extern (C) {
    ...
    void streamUDPReadCB(uv_udp_t* handle, ptrdiff_t nread,
            const(uv_buf_t)* buf, const(sockaddr)* addr, uint flags) {
        if (handle.data is null || addr is null)
            return;
        auto self = *cast(LibUVUDPLocalPoint*)handle.data;
        if (self is null)
            return;
