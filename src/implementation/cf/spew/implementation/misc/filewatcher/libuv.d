module cf.spew.implementation.misc.filewatcher.libuv;
import cf.spew.implementation.misc.filewatcher.base;
import devisualization.bindings.libuv;
import stdx.allocator : IAllocator, makeArray, dispose;

final class LibUVFileSystemWatcher : FileSystemWatcherImpl {
    ...
    this(string path, IAllocator alloc) {
        ...
        ctx.data = &self;
        libuv.uv_fs_event_start(&ctx, &libuvFSWatcherCB, thePathC.ptr,
                uv_fs_event_flags.UV_FS_EVENT_RECURSIVE);
    }
...
extern (C) {
    void libuvFSWatcherCB(uv_fs_event_t* handle, const(char)* filename, int events, int status) {
        ...
        char[] fullPath = watcher.alloc.makeArray!char((needSeperator ? 1
                : 0) + watcher.thePath.length + name.length);

        fullPath[0 .. watcher.thePath.length] = watcher.thePath[];
        if (needSeperator)
            fullPath[watcher.thePath.length] = '/';
