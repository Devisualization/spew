...
final class WindowImpl_X11 : WindowImpl,
    ...
    void setIcon(ImageStorage!RGBA8 from) @property {
        ...
        if (shared(UIInstance_X11_FreeDesktopNotify) uiInstance = cast(shared(UIInstance_X11_FreeDesktopNotify))instance) {
            if (uiInstance.haveNotificationWindow() &&
                (cast()uiInstance.taskbarTrayWindow).__handle == whandle) {
                ...
                uiInstance.drawSystray(cast(uint)from.width, cast(uint)from.height, imageData2.ptr);
            }
        }
