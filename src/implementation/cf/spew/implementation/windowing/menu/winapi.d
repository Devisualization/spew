module cf.spew.implementation.windowing.menu.winapi;
version (Windows):
import cf.spew.implementation.windowing.menu.base;
import cf.spew.implementation.windowing.window.winapi : WindowImpl_WinAPI;
import cf.spew.implementation.windowing.utilities.winapi : bitmapToImage_WinAPI,
    imageToBitmap_WinAPI;
...
import core.sys.windows.windows : HMENU, HBITMAP, AppendMenuA, CreateMenu,
    ModifyMenuA, RemoveMenu, DeleteMenu, DeleteObject, MENUITEMINFOA,
    GetMenuItemInfoA, HDC, GetDC, CreateCompatibleDC, DeleteDC, ReleaseDC,
    BITMAP, GetObjectA, ModifyMenuW, MF_BYCOMMAND, MF_POPUP, UINT_PTR,
    GetMenuStringW, MF_STRING, GetMenuState, MF_BITMAP, MF_SEPARATOR,
    MF_DISABLED, MF_ENABLED, DrawMenuBar;
...
final class MenuItemImpl_WinAPI : MenuItemImpl {
    ...
    override {
        Window_MenuItem addItem() {
            ...
            ModifyMenuW(parent, menuItemId, MF_BYCOMMAND | MF_POPUP,
                    cast(UINT_PTR)myChildren, textBuffer.ptr);
            return cast(Window_MenuItem)window.alloc.make!MenuItemImpl_WinAPI(window,
                    myChildren, this);
        }
        ...
        @property {
            ...
            managed!(ImageStorage!RGB8) image() {
                ...
                return managed!(ImageStorage!RGB8)(bitmapToImage_WinAPI(mmi.hbmpItem, hMemoryDC,
                        vec2!size_t(bm.bmWidth, bm.bmHeight), window.alloc),
                        managers(), window.alloc);
            }
            ...
            void image(scope ImageStorage!RGB8 input) {
                ...
                ModifyMenuA(parent, menuItemId, MF_BYCOMMAND | MF_BITMAP, 0,
                        cast(const(char)*)bitmap);
            ...
            managed!dstring text() {
                ...
                wchar[32] buffer;
                int length = GetMenuStringW(parent, menuItemId, buffer.ptr,
                        buffer.length, MF_BYCOMMAND);
                assert(length >= 0);
            ...
            void text(dstring input) {
                setText(input);
            }

            void text(wstring input) {
                setText(input);
            }
    ...
    void setText(T)(T input) if (isSomeString!T) {
        ...
        ModifyMenuW(parent, menuItemId, MF_BYCOMMAND | MF_STRING, menuItemId,
                cast(const(wchar)*)buffer.ptr);
