...
final class EventLoopManager_Impl : EventLoopManager_Base {
    ...
    this(shared(ISharedAllocator) allocator = processAllocator(),
            ThreadID mainThreadID = Thread.getThis().id) shared {
        super(allocator, mainThreadID);

        this.hintSourceTimeout = 0.seconds;
        this.onErrorDelegate = &onErrorDelegateDefaultImpl;

        this.sources = cast(shared)DynamicArray!(EventLoopSource,
                shared(ISharedAllocator))(allocator);
        this.consumers = cast(shared)DynamicArray!(EventLoopConsumer,
                shared(ISharedAllocator))(allocator);
        ...
        this.threadData = cast(shared)HashMap!(ThreadID, InternalData*,
                shared(ISharedAllocator))(allocator);
    }
    ...
    void clearConsumers() shared {
        synchronized (mutex_consumersAlter) {
            while (!(cast()consumers).empty)
                (cast()consumers).removeBack;
        }
    }

    void clearSources() shared {
        synchronized (mutex_sourcesAlter) {
            while (!(cast()sources).empty)
                (cast()sources).removeBack;
        }
    }
    ...
    protected override {
        ...
        void* initializeImpl(ThreadID threadId) shared {
            ...
            foreach (source; cast()sources) {
                if ((isOnMainThread && (cast(shared)source).onMainThread) ||
                        (!isOnMainThread && (cast(shared)source).onAdditionalThreads)) {
                    sourceCount++;
                }
            }

            shared(EventLoopConsumer)[] allConsumers = allocator.makeArray!(
                    shared(EventLoopConsumer))((cast()consumers).length);
            ...
            foreach (source; cast()sources) {
                ...
                foreach (consumer; cast()consumers) {
                    if (((isOnMainThread && (cast(shared)consumer).onMainThread) ||
                            (!isOnMainThread && (cast(shared)consumer).onAdditionalThreads)) &&
                            ((cast(shared)consumer).pairOnlyWithSource.isNull ||
                                (cast(shared)consumer).pairOnlyWithSource.get == (cast(shared)source)
                                .identifier)) {
                ...
                ret.instances[i].consumers = allocator.makeArray!(
                        shared(EventLoopConsumer))(allConsumersSlice.length);
                ...
                foreach (instance; data.instances) {
                    ...
                    while (instance.retriever.nextEvent(event)) {
                        ...
                        if (event.type.value > 0) {
                            ...
                            if ((cast()threadsState)[threadId] == ThreadState.Stop)
                                break;
                        }
                    }
                }
